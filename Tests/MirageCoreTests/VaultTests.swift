...
struct KeychainErrorTests {
    ...
    func itemNotFound() {
        let error = VaultError.itemNotFound(key: "myKey")
    ...
    func saveFailed() {
        let error = VaultError.saveFailed(key: "token", status: -25299)
    ...
    func loadFailed() {
        let error = VaultError.loadFailed(key: "secret", status: -25300)
    ...
    func deleteFailed() {
        let error = VaultError.deleteFailed(key: "old", status: -25244)
    ...
    func encodingFailed() {
        ...
        let error = VaultError.encodingFailed(key: "data", underlyingError: TestError())
    ...
    func decodingFailed() {
        struct TestError: Error {}
        let error = VaultError.decodingFailed(key: "data", underlyingError: TestError())
        #expect(error.refcode == "WWKF")
        #expect(error.underlyingError is TestError)
    }

    @Test("conforms to MirageError")
    func mirageErrorConformance() {
        let error: any MirageError = VaultError.itemNotFound(key: "test")
