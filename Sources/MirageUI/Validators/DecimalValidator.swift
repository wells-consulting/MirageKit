...
public final class DecimalValidator: Validator {
    ...
    typealias CustomValidator = (Decimal) -> Bool
    ...
    public func validate(_ text: String) -> ValidationResult<Decimal, Violation> {
        let value = formatter.number(from: text)?.decimalValue
    ...
    public enum Requirement: Equatable, Hashable {
        case validFormat
        case minValue(Decimal)
        case maxValue(Decimal)
