...
//
    ...
    nonisolated(unsafe) var crashLogPathC: UnsafeMutablePointer<Int8>?

    // C-compatible signal handler (signal-safe).
    @_cdecl("crashSignalHandler")
    nonisolated func crashSignalHandler(_ signum: Int32) {
    ...
    public actor CrashLog {
        ...
        private init() {
            ...
            //
            // Safe to force-unwrap: this code only runs on Darwin (gated by
            // `#if canImport(Darwin)`). The documents directory is guaranteed to
            // exist on Apple platforms. If it didn't, the app sandbox is
            // fundamentally broken and nothing would work.
