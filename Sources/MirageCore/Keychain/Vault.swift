...
public struct Vault: Sendable {
    ...
    public init(
        ...
        self.service = service ?? (Bundle.main.bundleIdentifier ?? "com.miragekit.keychain")
    ...
    public func load<T: Codable>(_ type: T.Type, forKey key: String) throws -> T {
        ...
        guard status != errSecItemNotFound else {
            throw VaultError.itemNotFound(key: key)
        }

        guard status == errSecSuccess, let data = result as? Data else {
            throw VaultError.loadFailed(key: key, status: status)
        }

        do {
            return try Jayson.shared.decode(T.self, from: data)
        } catch {
            throw VaultError.decodingFailed(key: key, underlyingError: error)
        }
    }
    ...
    public func save(_ value: some Codable, forKey key: String) throws {
        ...
        do {
            ...
            throw VaultError.encodingFailed(key: key, underlyingError: error)
        }
        ...
        guard status == errSecSuccess else {
            throw VaultError.saveFailed(key: key, status: status)
        }
    }
    ...
    public func delete(key: String) throws {
        ...
        guard status == errSecSuccess || status == errSecItemNotFound else {
            throw VaultError.deleteFailed(key: key, status: status)
        }
    }
